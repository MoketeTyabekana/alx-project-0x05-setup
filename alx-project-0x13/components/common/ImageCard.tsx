...
const ImageCard: React.FC<GeneratedImageProps> = ({ imageUrl, prompt, width, action }) => {
  return (
    <div onClick={() => action(imageUrl)} className="mt-6 p-4 shadow-md rounded-xl hover:cursor-pointer">
      ...
      <p className={`${width ? 'text-xs' : 'text-lg'} text-gray-700 mb-4 truncate`}>{prompt}</p>
    </div>
  )
}
